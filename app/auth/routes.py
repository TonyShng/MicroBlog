...
# 	posts = current_user.followed_posts().paginate(page, app.config['POSTS_PRE_PAGE'], False)
...
# 	page = request.args.get('page', 1, type=int)
# 	posts = user.posts.order_by(Post.timestamp.desc()).paginate(page, app.config['POSTS_PRE_PAGE'], False)
...
# def explore():
# 	page = request.args.get('page', 1, type=int)
# 	posts = Post.query.order_by(Post.timestamp.desc()).paginate(page, app.config['POSTS_PRE_PAGE'], False)
...
def reset_password(token):
	...
	if form.validate_on_submit():
		...
		return redirect(url_for('auth.login'))
	return render_template(_('reset_password.html'), form=form)
